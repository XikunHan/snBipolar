...
f_manhattan_annotation_BD <-
  function(infile,
    ...
    log10.cell <- function(log, pval.chunks) {
      idx <-
        pvals.cells.index$id[log >= pvals.cells.index$LP &
                               log < pvals.cells.index$UP][[1]]
      return (idx)
    }
    ...
    chrpos.cell <- function(chr, position, chr.chunks) {
      slice <-
        chr.chunks[chr.chunks$chr == chr &
                     chr.chunks$s < position & chr.chunks$e > position, ]
      
      return (slice$posid)
    }
    
    if (rebuild == T) {
      ## rebuild the heatmap matrix and other datastructures if the flag is set
      message("Rebuilding matrix")
      ## read the gwas results
      message("Reading the GWAS results...")
      d <- read.table(infile, header = T)
      
      d <- d[, c(chrname, posname, pvalname, frqname, conseqname)]
      d
      ...
      waitifnot(
        is.numeric(d$chr),
        "chr column in gwas data should be numeric, please check if encoded X, or with `chr` prefix"
      )
      ...
      snp.info <-
        snp.info[order(snp.info$chr, snp.info$pos, decreasing = F), ]
      
      ## read the config data
      # config<-read.table(configfile,sep="\t", header =T,stringsAsFactors = F, skip=10)
      
      config <- fread(configfile,
                      sep = "\t",
                      header = T,
                      skip = 10)
      setDF(config)
      ...
      pvals <-
        seq(from = 0, to = max.pval, by = pval.split) # max(-log10(d$Pvalue))
      pvals.cells.index <-
        data.frame(
          id = 1:length(pvals),
          LP = pvals,
          UP = c(pvals[2:length(pvals)], max.pval)
        )
      ...
      if (debugflag == T) {
        ## Generate the log file if the flag is set.
      ...
      for (chr in 1:lastchr) {
        message(paste("Processing chromosome ", chr, "\r", sep = ""),
                appendLF = F)
        ...
        chunks <-
          seq(
            from = min(chr.slice$pos),
            to = max(chr.slice$pos),
            by = pos.split
          )
        ...
        mdat <-
          matrix(0, nrow = length(pvals), ncol = length(chunks) - 1)
        
        for (i in 1:(length(chunks) - 1)) {
          ## get slice of gwas input for each of the position bins
          slice <-
            chr.slice[chr.slice$pos >= chunks[i] & chr.slice$pos < chunks[i + 1], ]
          
          for (j in 1:length(pvals)) {
            ## get slice of gwas input for each of the pvalue bins (and position bins)
            if (j == length(pvals)) {
              ## last element in pval array - include all variants gt then this
              p.val.slice <-
                slice[-log10(slice$Pvalue) >= pvals[j], ]
            }
            else{
              ## otherwise slice between the current and next values in the pval array
              p.val.slice <-
                slice[-log10(slice$Pvalue) >= pvals[j] &
                        -log10(slice$Pvalue) < pvals[j + 1], ]
            }
            ...
            conseq.len <-
              dim(p.val.slice[p.val.slice$conseq == 1, ])[1] ## test if any high consequences
            ## determine the number of variants in the slice, which have MAF less than 5%
            maf.len <-
              dim(p.val.slice[p.val.slice$FRQ <= MAF, ])[1] ## test if any MAF < 5%
            ...
            if (len == 0) {
              ...
              if (pvals[j] <= -log10(FDR)) {
                ...
                for (k in 1:length(config$idx)) {
                  if (config$type[k] == "val") {
                    ...
                    if (len >= config$min.count[k]) {
                      ## check that the current cell length is gt or eq than the config min count
                      if (is.na(config$max.count[k])) {
                        ## if the config max is NA then accept length condition
                        len.chk <- TRUE
                      }
                      else if (len <= config$max.count[k]) {
                        ## if the current cell length is lt the config max count then accept the length condition
                        len.chk <- TRUE
                      }
                    }
                    
                    ## check HIGH impact
                    if (config$conseq[k] == TRUE &&
                        conseq.len > 0) {
                      conseq.chk <-
                        TRUE
                    } ## accept if high impact is active in config and there are 1 or more high impact variants within the cell
                    else if (config$conseq[k] == FALSE &&
                             conseq.len == 0) {
                      conseq.chk <-
                        TRUE
                    } ## accept if high impact is inactive and there are 0 high impact variants within the cell.
                    
                    ## check MAF
                    if (config$maf[k] == TRUE &&
                        maf.len > 0) {
                      maf.chk <-
                        TRUE
                    } ## accept if MAF 5% active and there is one or more variant with MAF lt 5% in the cell
                    else if (config$maf[k] == FALSE &&
                             maf.len == 0) {
                      maf.chk <-
                        TRUE
                    } ## accept if MAF 5% active and there are 0 variants with MAF lt 5% in the cell
                    
                    if (len.chk == TRUE &&
                        conseq.chk == TRUE &&
                        maf.chk == TRUE) {
                      ## if all three clauses are correct then accept the idx for the config and exit the for loop
                      idx <- config$idx[k]
                      
                      if (config$report[k] == TRUE) {
                        ## if reporting is active for the config then add an entry to the pos.interest table.
                        tmp.df <- data.frame(
                ...
                if (idx == 0) {
                  ## if no idx was found in the annotations, generate a remaining idx (max idx in annotations + 1)
                ...
                if (len > max.cellcount) {
                  max.cellcount <-
                    len
                } ## assign the maximum cell count if length of current cell is gt than current
              }
            }
            
            idx.count[idx] <-
              (idx.count[idx] + 1) # increment the block count for each index
            
            mdat[j, i] <- idx
            
            if (debugflag == T) {
              ## log information for current cell.
              ...
              if (idx != oddchridx ||
                  idx != evenchridx ||
                  idx != 0) {
                ## generate the log file, dont log the greyed out regions
                cat(
                  chr,
                  chunks[i],
                  chunks[i + 1],
        ...
        chr.matrix.len$mid[chr] <-
          chr.matrix.len$cumm[chr] - (chr.matrix.len$length[chr] / 2)
        
        ## bind to the final matrix
        final <- cbind(final, mdat)
        
        tmp.chunks <-
          as.data.frame(matrix(nrow = length(chunks) - 1, ncol = 4))
        names(tmp.chunks) <- c("posid", "chr", "s", "e")
        
        for (j in 1:dim(tmp.chunks)[1]) {
          ## assign position chunk indexes to table
          #print(j)
      ...
      if (debugflag == T) {
        ## close the log file if debuging is active.
        close(logfile)
      }
      ...
      for (i in 1:length(snp.info$markername)) {
        ## assign a position cell for each of the variants in the SNP information table.
        snpcells[i] <-
          chrpos.cell(snp.info$chr[i], snp.info$pos[i], pos.chunks)
      }
      
      snp.info$chunk <- snpcells
      ...
      if (dim(pos.interest)[1] > 0) {
        ## if there are any positions of interest assign the cell positions on the heatmap
        pos.interest$pvalidx <- log10.index(pos.interest$log10pval)
        pos.interest$pos.idx <- -1
        
        for (i in 1:length(pos.interest$marker)) {
          ## assign a position cell for each of the cells of interest
          pos.interest$pos.idx[i] <-
            chrpos.cell(pos.interest$chr[i], pos.interest$pos[i], pos.chunks)
        }
      }
    ...
    peak.val <-
      ceiling(max.cellcount / 100) * 100 ## nearest 10,000 to max cell value
    ...
    for (k in 1:length(col.text)) {
      if (config$type[k] == "val") {
        ...
        if (is.na(max.count)) {
          ## if the max count is assigned to NA then the highest cell count (with approp ceiling) is used
          max.count <- peak.val
        }
        ...
        if (config$conseq[k] == FALSE &&
            config$maf[k] == FALSE) {
          ## if both MAF and HIGH impact are disabled - do not add any text
          ##col.text[k]<-col.text.tmp
        }
        else if (config$conseq[k] == TRUE &&
                 config$maf[k] == TRUE) {
          ## if both MAF and HIGH impact are active - print BOTH
          col.text.tmp <- paste(col.text.tmp, " (BOTH)", sep = "")
        }
        else if (config$conseq[k] == TRUE &&
                 config$maf[k] == FALSE) {
          ## if only HIGH impact is active - print HIGH impact.
          col.text.tmp <- paste(col.text.tmp, " (HIGH impact)", sep = "")
        }
        else if (config$conseq[k] == FALSE &&
                 config$maf[k] == TRUE) {
          ## if only MAD is active - print MAF
          col.text.tmp <-
            paste(col.text.tmp, " (MAF < ", MAF, ")", sep = "")
        }
        else{
          ## all possible scenarios are covered - should not get here
          warning("Should not get here!\n")
          stopifnot(FALSE)
        }
        col.text[k] <-
          paste(col.text.tmp, " (", idx.count[k], ")", sep = "")
      }
      else{
        ## otherwise print the config type.
    ...
    if (max(m$val) > max(config$idx)) {
      ## if there are any cells which do not have a category add them to the remaining category.
      col.brks <- c(col.brks, max(m$val))
      col.discrete <-
        c(col.discrete, "orange") ## hard coded colour for remaining.
      col.text <-
        c(col.text, paste("Remaining (", idx.count[max(m$val)], ")", sep = ""))
    }
    ...
    if (showgenes == FALSE) {
      ## if show genes flag is not set then put all the genes in the table
      snp.info$novel = TRUE
    }
    ...
    snp.info$logP <- -log10(snp.info$Pvalue)
    snp.info
    ...
    # table(is.na(mm$logP))
    # table(is.na(mm$markername))
    #
    # table(duplicated(mm$markername[!is.na(mm$markername)]))
    #
    # mm$logP <- ifelse(is.na(mm$markername), NA, mm$pval)
    #
    # head(mm)
    #
    # p.val.cell_2(NA)
    # sapply(NA, p.val.cell_2)
    # p.val.cell(1.621e-09)
    # p.val.cell(2.642e-11)
    
    mm$logP <- sapply(mm$Pvalue, p.val.cell_2)
    
    df_anno <-
      mm[(!duplicated(mm$markername)) & (!is.na(mm$markername)) ,]
    ...
    df_anno$gene <- paste0("italic('", df_anno$gene, "')")
    ...
    main.core <- ggplot(data = mm, aes(x = pos, y = pval)) +
      # geom_point()
      ...
      geom_point(data = df_anno, aes(pos, logP), color = "red")  +
    ...
    if (dim(snp.info.known)[1] > 0) {
      ...
      if (dim(repel.df[repel.df$pvalidx > log10.index(max.pval), ])[1] > 0) {
        repel.df[repel.df$pvalidx > log10.index(max.pval), ]$pvalidx <-
          log10.index(max.pval)
      }
      
      final.repel.plot <- main.core +
        geom_label_repel(
          data = repel.df,
          aes(posidx, pvalidx, label = marker),
          size = textsize,
          force = 1,
          nudge_y = 10,
          nudge_x = 10,
    ...
    if (dim(pos.interest)[1] > 0) {
      main.core <-
        main.core + geom_label_repel(
          data = pos.interest,
          aes(pos.idx, pvalidx, label = marker),
          size = textsize,
          force = 5,
          nudge_y = 10,
          nudge_x = 10,
    ...
    #  title.pos <- c(table.pos(20)+1, table.pos(14.5)+1, table.pos(10.5)+1, table.pos(8)+1,table.pos(6.5)+1,table.pos(3.5)+1)
    title.pos <-
      c(
        table.pos(20) + 1,
        table.pos(18) + 1,
    ...
    table1 <- ggplot(data = m, aes(x = pos, y = pval)) +
      ...
      scale_x_continuous(
        ...
        trans = "reverse",
        position = "top"
      ) + #, minor_breaks=chr.matrix.len$cumm) +
      scale_y_continuous(
        "",
        labels = rep("", length(y.labels)),
        breaks = y.breaks,
        expand = c(0, 0),
        trans = "reverse",
        position = "right"
    ...
    #
    segment.indexes <-
      c(table.pos(20) + 1, table.pos(21) + 1, table.pos(22) + 1)
    chr.num.pos <- (table.pos(22) + 1)
    brks.pos <- (table.pos(21) + 1)
    ...
    text.pos <-
      seq(
        from = xmin,
        to = xmax,
        length.out = length(snp.info.novel$markername) + 1
      )
    ...
    if (dim(snp.info.novel)[1] > 0) {
      ...
      table2 <- table1 +
        annotate(
          ...
          x = text.pos1,
          y = title.pos[1],
          label = as.character(snp.info.novel$markername),
          angle = 0,
          size = textsize,
          hjust = 0
        ) + ## markername
        annotate(
          ...
          x = text.pos1,
          y = title.pos[2],
          label = as.character(snp.info.novel$ID),
          angle = 0,
          size = textsize,
          hjust = 0
        ) + ## markername
        annotate(
          ...
          label = as.character(format(
            round(snp.info.novel$eaf, 2), nsmall = 2
          )),
          angle = 0,
          size = textsize,
          hjust = 0
        ...
        annotate(
          ...
          x = text.pos1,
          y = title.pos[4],
          label = as.character(format(round(
            snp.info.novel$OR, 2
          ), nsmall = 2)),
          angle = 0,
          size = textsize,
        ...
        annotate(
          ...
          x = text.pos1,
          y = title.pos[5],
          label = as.character(formatC(
            snp.info.novel$Pvalue,
            format = "E",
            digits = 2
          )),
          angle = 0,
        ...
        annotate(
          ...
          x = text.pos1,
          y = title.pos[6],
          label = as.character(snp.info.novel$gene),
          angle = 0,
          size = textsize,
          hjust = 0,
          fontface = 'italic'
        ...
        annotate(
          "segment",
          x = text.pos1,
          xend = snpcells[snp.info$novel == TRUE],
          y = segment.indexes[1],
          yend = segment.indexes[2],
          colour = RColorBrewer::brewer.pal(12, "Set3")[1],
          linetype = "dashed",
          size = 0.5
        ) +  ## segment from midpoint to table row
        annotate(
          "segment",
          x = snpcells[snp.info$novel == TRUE],
          xend = snpcells[snp.info$novel == TRUE],
          y = segment.indexes[2],
          yend = segment.indexes[3],
          colour = RColorBrewer::brewer.pal(12, "Set3")[4],
          linetype = "dashed",
          size = 0.5
        ) + ## segment from axis to mid point
        annotate(
          "text",
          x = chr.matrix.len$mid,
          y = chr.num.pos,
          label = as.character(1:lastchr),
          angle = 0,
          size = 3.5,
          hjust = 0
        ) + ## chromosome labels
        annotate(
          "segment",
          x = chr.matrix.len$cumm,
          xend = chr.matrix.len$cumm,
          y = brks.pos,
          yend = segment.indexes[3],
          colour = "black",
          linetype = "solid"
    ...
    p <- qplot(1:10, 1:10, colour = I("white")) +
      annotation_custom(
        grob = ggplotGrob(final.plot),
        xmin = 0 ,
        xmax = manh.max,
        ymin = 1,
        ymax = 10
